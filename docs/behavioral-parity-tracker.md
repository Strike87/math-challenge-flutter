...
| BPT-002 | AdMob banner/interstitial/rewarded | Ported local behavior | `index.html:11444`, `index.html:11542`, `index.html:11675`, `index.html:11727` | `pubspec.yaml:9`, `lib\services\admob.dart:5`, `lib\engine\game_state.dart:986`, `lib\widgets\modals.dart:2277`, `test\admob_parity_test.dart:29` | VERIFIED / Frozen for scoped local ad behavior slice; device QA pending | Confirmed | Codex | Unit / Device | Yes | RT-051 | Local BPT-002 behavior is verified: banner eligibility is limited to Number Type and Player Setup; `adsRemoved` disables banner, interstitial, and rewarded prompts; completed games persist `mc_adGameCount` and request interstitial every third game; rewarded success grants exactly +10 coins only after the ad-service reward result; rewarded failures/unavailable/cancel paths grant nothing; 300,000ms cooldown persists in `mc_lastRewardedAt`; unavailable ad service and blocked native-release dev simulation grant nothing and do not crash; Families-safe child-directed request policy is exposed by the ad service; and the configured Android App ID / banner / interstitial / rewarded defaults use the real AdMob IDs. Validation passed on 2026-06-30: `flutter test test\admob_parity_test.dart --reporter compact`, `flutter test --reporter compact`, and `flutter analyze`. Manual release QA still remains for Android device ad loading/no-fill behavior and AdMob/Families console proof. Repo note: this verification claim applies only to the BPT-002 files and local behavior touched here, not to unrelated pending changes. |
...
| BPT-007 | Persistence schema parity | Ported behavior | `index.html:6607`, `index.html:6726`, `index.html:6744`, `index.html:10402`, `index.html:10685` | `lib\services\storage.dart:22`, `lib\engine\game_state.dart:364`, `lib\engine\game_state.dart:447`, `lib\engine\game_state.dart:492`, `lib\services\settings.dart:49`, `test\persistence_schema_test.dart:52`, `docs\persistence-schema-audit.md` | VERIFIED | Confirmed | Codex | Unit | Yes | RT-005 | Active schema decisions are source-mapped and tested. RT-005 covers original legacy achievements, skills, number-type unlocks, player data, shop ownership, settings persistence, same-key JSON primitives, avatar custom fallback, daily date/challenge normalization, login streak date migration, and raw `mc_lowPerf`. Service-only IAP/AdMob keys are documented in the schema audit and covered by BPT-001/BPT-002 local behavior tests. `flutter test test\persistence_schema_test.dart`, `flutter test`, and `flutter analyze` passed on 2026-06-27. |
...
| `mc_adsRemoved`, `mc_iapDeliveredTxs`, `mc_lastRewardedAt`, `mc_adGameCount` | Active | Monetization/ad keys are covered by BPT-001/BPT-002 local behavior tests. Real device/store proof remains release QA. |
...
9. BPT-001 / BPT-002: Completed locally; finish Play Console, AdMob device, and Families release QA.
...
| RT-011 | BPT-010 | Shop ownership | Unit | Permanent avatar/hat unlocks, repeatable Power Pack and Extra Life consumables, +20 daily coin bonus once per day, locked avatar visibility, insufficient coins, durable save, and reload persistence match the local shop contract. |
...
| Shop / local economy | BPT-010 | Frozen for scoped local shop-ownership slice | RT-011 | Permanent unlocks, repeatable consumables, rewarded-ad unavailable no-grant guard, avatar picker gating, insufficient-coin handling, and persistence are verified for the BPT-010 files/behavior only. Do not refactor unless BPT-010 is reopened with a verified bug. |
...
| AdMob | BPT-002 | Frozen for scoped local ad behavior slice; device QA pending | RT-051 | Local banner gating, interstitial cadence/persistence, rewarded grants/cooldown/failure paths, remove-ads gating, unavailable-service safety, native-release dev-simulation blocking, Families-safe request policy, and real Android AdMob default IDs are covered by RT-051/app config review. Android device loading/no-fill behavior and AdMob/Families console proof remain release QA. Do not refactor unless BPT-002 is reopened with a verified bug. |
...
Behavioral parity implementation is complete locally.

Remaining work is release-readiness proof:

- Android release build proof.
- Play Console IAP internal-test proof.
- Real AdMob/test-ad device proof.
- Families policy review.

Release QA note:

- Native IAP is wired through `NativeIapPurchaseAdapter`; Play Console internal-test purchases still need real-device proof.
