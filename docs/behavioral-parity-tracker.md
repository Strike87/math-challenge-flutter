...
| BPT-002 | AdMob banner/interstitial/rewarded | Ported local behavior | `index.html:11444`, `index.html:11542`, `index.html:11675`, `index.html:11727` | `pubspec.yaml:9`, `lib\services\admob.dart:5`, `lib\engine\game_state.dart:986`, `lib\widgets\modals.dart:2277`, `test\admob_parity_test.dart:29` | VERIFIED / Frozen for scoped local ad behavior slice; device QA pending | Confirmed | Codex | Unit / Device | Yes | RT-051 | Local BPT-002 behavior is verified: banner eligibility is limited to Number Type and Player Setup; `adsRemoved` disables banner, interstitial, and rewarded prompts; completed games persist `mc_adGameCount` and request interstitial every third game; rewarded success grants exactly +100 coins only after the ad-service reward result; rewarded failures/unavailable/cancel paths grant nothing; 300,000ms cooldown persists in `mc_lastRewardedAt`; unavailable ad service and blocked native-release dev simulation grant nothing and do not crash; Families-safe child-directed request policy is exposed by the ad service. Validation passed on 2026-06-29: `flutter test test\admob_parity_test.dart --reporter compact`, `flutter test --reporter compact`, and `flutter analyze`. Manual release QA still remains for real `google_mobile_ads` unit IDs, Android device ad loading/no-fill behavior, and AdMob/Families console proof. Repo note: this verification claim applies only to the BPT-002 files and local behavior touched here, not to unrelated pending changes. |
...
| AdMob | BPT-002 | Frozen for scoped local ad behavior slice; device QA pending | RT-051 | Local banner gating, interstitial cadence/persistence, rewarded grants/cooldown/failure paths, remove-ads gating, unavailable-service safety, native-release dev-simulation blocking, and Families-safe request policy are covered by RT-051. Real `google_mobile_ads` unit IDs, Android device loading/no-fill behavior, and AdMob/Families console proof remain release QA. Do not refactor unless BPT-002 is reopened with a verified bug. |
